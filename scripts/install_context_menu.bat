...
if not exist "%SCRIPT_DIR%quick_restore.py" (
    echo [错误] 未找到 quick_restore.py
    echo 请确保脚本位于 scripts\ 目录中
    pause
    exit /b 1
)
...
:: 查找 pythonw.exe（无控制台窗口，不需要弹出窗口）
echo.
echo [2/4] 查找 pythonw.exe...
set "PYTHONW_CMD="
for /f "delims=" %%i in ('where pythonw 2^>nul') do set "PYTHONW_CMD=%%i"
if "%PYTHONW_CMD%"=="" (
    echo [警告] 未找到 pythonw.exe，将使用 python
    ...
    echo [OK] 使用 python（运行时可能短暂显示控制台窗口）
) else (
    echo [OK] %PYTHONW_CMD%
)
...
set "RESTORE_CMD=\"%PYTHONW_CMD%\" \"%SCRIPT_DIR%quick_restore.py\" \"%%1\""
...
:: 安装"一键还原"
echo   - 一键还原...
reg add "%REG_BASE%\QuickRestore" /ve /t REG_SZ /d "一键还原" /f >nul 2>&1
reg add "%REG_BASE%\QuickRestore" /v Icon /t REG_SZ /d "%ICON_PATH%" /f >nul 2>&1
reg add "%REG_BASE%\QuickRestore\command" /ve /t REG_SZ /d "%RESTORE_CMD%" /f >nul 2>&1
