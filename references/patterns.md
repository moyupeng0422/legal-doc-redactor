...
- **v1.2.0 (2026-02-13)**: 添加自定义脱敏类型、优先级机制（黑名单>白名单>脱敏类别）、冲突检测机制
...
白名单、黑名单和自定义脱敏类型允许用户精确控制特定内容的脱敏行为，弥补自动识别的不足。
...
**优先级示例**：
- 黑名单有"AA"，白名单有"AABB" → "AA"仍会被识别（黑名单优先）
- 白名单有"计划"，脱敏识别到"股权激励计划" → 不被识别（白名单覆盖）

### 白名单（不脱敏）

**用途**: 某些内容虽然匹配了脱敏规则，但用户希望保留原文。

**覆盖检查**: 白名单采用覆盖检查（包含关系），如果脱敏内容包含白名单中的任何一项，则跳过脱敏。

**示例场景**:
- 白名单有"计划" → "股权激励计划"不被识别为项目
- 白名单有"公司" → "某某公司"不被识别为公司名

**操作方式**:
1. 点击"白名单"按钮
2. 输入内容并添加
3. 该内容及其包含该内容的匹配项将不会被脱敏
...
**冲突示例**:
- 添加"AA"到白名单，但"AA"已在黑名单中 → 显示冲突提示
- 添加"BB"到自定义类型，但"BB"已在白名单中 → 显示冲突提示

### 防冲突机制

**删除脱敏项时**:
- 如果内容来自黑名单/自定义类型 → 显示警告，需手动删除后才能加入白名单
- 如果内容来自其他来源 → 询问是否加入白名单
